    ...
    public partial class Frm_PORT_S : FrmBaseSet
    {
        ...
        protected override string yssDoSetFormOperMVC(ArrayList pojoList, ClsEnums.StatusSetting status)
        {
            if (cboProdState.Value != null)
            {
                ...
                ////update zhanygongzhao 20170711 STORY #43773 太平资管：产品提前清算及到期确认时更新到期清算时间
                //// 添加状态判断
        ...
        public override AuditableParamPojo faceInfoToObjMVC()
        {
            ...
            try
            {
                ...
                //// 简单处理，待和估值中内容整合 
                port.D_COLSE_ACC = Convert.ToDateTime("9998-12-31");
        ...
        public override void showInfoMVC(BasePojo pojo)
        {
            ...
            try
            {
                if (this.showInfoMvc == true)
                {
                    ...
                    ////ctlCboClear();
					////update zhangyongzhao 2017-0711 STORY #43773 太平资管：产品提前清算及到期确认时更新到期清算时间
					ctlCboClear();
                    //// 对窗体控件一一赋值
                    this.txtPortCode.Text = port.C_PORT_CODE;
                    this.txtPortCNName.Text = port.C_PORT_NAME;
        ...
        private void cboPortLever_SelectedValueChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                ////ctlCboClear();
				////update zhangyongzhao 2017-711 STORY #43773 太平资管：产品提前清算及到期确认时更新到期清算时间
                ctlCboClear();
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// 控制清算日期方法
        /// update zhangyongzhao 2017-711 TASK #318197 代码开发-太平资管：产品提前清算及到期确认时更    
        /// </summary>
        private void ctlCboClear()
        {
            if (isCreateCopy)
            ...
            if (cboProdState.Value != null)
            {
                YssProductInfo.Support.Ab.Port.Pojo.Port port = (YssProductInfo.Support.Ab.Port.Pojo.Port)this.yssGetBaseSelTypeItemMVC();

                if (status != ClsEnums.StatusSetting.YssAdd && port != null)
                {
                    dtpClear.setDateTime(port.D_CLEAR);
                    this.dtpExpirationDate.setDateTime(DateTime.Parse(port.D_CLOSE));
                }

                FAST.Core.Context.ClsEnums.PD_STATUS statusPd = (ClsEnums.PD_STATUS)Enum.Parse(typeof(ClsEnums.PD_STATUS), cboProdState.Value);
                if (statusPd == ClsEnums.PD_STATUS.PS5 && port != null)
                {
                    dtpClear.setDateTime(port.D_CLEAR);
                    clsInterface.setControlsStatus(tbMain, ClsEnums.StatusSetting.YssBrow);
                    this.cboPort.YssReadOnly = true;
                    dtpExpirationDate.ReadOnly = false;
                    dtpClear.yssEnabled = true;
