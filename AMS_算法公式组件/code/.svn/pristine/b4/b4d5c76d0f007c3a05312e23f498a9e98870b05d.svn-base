...
(function ($) {
	$.addFlex = function (t, p) {
		...
		p = $.extend({ //apply default properties
			...
			checkedArray:new Array(0)//需要勾选的行的ID，tr的ID
		}, p);
...
//end modify 增加checkbox					
			...
			addData: function (data) { //parse data
				...
				if (p.dataType == 'json') {
					var preArray = new Array(0); //要追加到前面的行
...
//Orlando 20140922  增加checkbox						
						if (p.checkbox) {  
                            ...
                            var objTr = $(tr);
                            var cthch;
                            if($.inArray(tr.id, p.checkedArray) != -1){
                            	cthch = $('<input type="checkbox" value="' + row.id +'" checked="true" />');  
                            	objTr.addClass('trSelected'); 
                            }else{
                            	cthch = $('<input type="checkbox" value="' + row.id +'"/>');  
                            }
...
//end modify 增加checkbox						
					...
					}).dblclick(function(e){//添加双击事件
						if (p.onRowDblclick){
	                         var rowData = new Object();
	                         if(p.colModel){
		                         $.each($(this).find('div'),function(i){
		                          //把当前行每一格的值放进rowData,取: rowData.列名
		                          rowData[p.colModel[i].name] = $(this).text();
		                         });
	                         }
