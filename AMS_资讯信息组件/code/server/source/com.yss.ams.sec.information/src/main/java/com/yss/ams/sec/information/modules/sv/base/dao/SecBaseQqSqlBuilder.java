 ...
 */
public class SecBaseQqSqlBuilder implements SQLBuilder
{

	/**
	 * 返回 根据前台查询条件获取期权品种信息数据总数量 的SQL
	 * @param paraNameList
	...
	public String getQueryConditionCountSql(List<String> paraNameList) throws Exception
	{
		...
		try
		{
			...
			//buf.append("left join (select c_da_code,c_sec_var_code from T_P_BI_SEC_VAR where N_CHECK_STATE =1)b on a.c_sec_var_code = b.c_sec_var_code");
			buf.append("left join (select c_da_code,c_sec_var_code from V_S_DA_SEC_VAR )b on a.c_sec_var_code = b.c_sec_var_code");
			buf.append(" where b.c_sec_var_code like 'QQ%' ");
