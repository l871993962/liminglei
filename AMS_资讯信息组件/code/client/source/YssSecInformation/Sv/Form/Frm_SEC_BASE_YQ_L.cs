...
{
    /// <summary>
    /// 功能简介：远期品种信息浏览界面，负责远期信息的显示和查询功能的实现 
    /// 创建版本：V4.5.0.1
    /// 创建人： liuliang
    /// 创建日期： 2012.05.29
    /// </summary>
    public partial class Frm_SEC_BASE_YQ_L : FrmBaseList
    {
        /// <summary>
        /// dataService
        /// </summary>
        ...
        /////// list界面加载A区数据，子类重写
        /////// </summary>
        /////// <author>wuwenlan 2012.05.29</author>
        ...
        public string yssGetListCond()
        {
            ...
            quyStrUtil.addQuyCon("C_DA_CODE", "C_DA_CODE", "YQ", ClsConstant.SQL_RA_HYPHEN_LIKE, ClsConstant._LIKE_MARCH_LEFT);
            ...
            if (this.txtSecCode.Text.Trim().Length != 0)
            {
                ...
                quyStrUtil.addQuyCon("C_SEC_CODE", this.txtSecCode.Text, ClsConstant.SQL_RA_HYPHEN_LIKE);
            }
            ...
            if (this.txtSecCode.Text.Trim().Length != 0)
            {
                quyStrUtil.addQuyCon("C_SEC_ISIN_CODE", this.txtSecCode.Text, ClsConstant.SQL_RA_HYPHEN_LIKE);
            }
            ...
            if (this.selSecVar.Value != null)
            {
                ////cond += " and a.C_SEC_VAR_CODE like  '" + this.cboSecVar.Value + "%'";
                quyStrUtil.addQuyCon("C_SEC_VAR_CODE", this.selSecVar.Value, ClsConstant.SQL_RA_HYPHEN_LIKE);
            }

            string search = this.yssBuildLeftCheckRowsStr("exchange"); // 获取A区选中的行
        ...
        protected override Dictionary<string, string> OnGetParaAssemble(Dictionary<string, string> paraDict)
        {
            ...
            if (this.txtSecCode.Text.Trim().Length != 0)
            {
                paraDict.Add("C_SEC_CODE", "%" + this.txtSecCode.Text.Trim() + "%");
            }
            ...
            if (this.txtSecCode.Text.Trim().Length != 0)
            {
                paraDict.Add("C_SEC_ISIN_CODE", "%" + this.txtSecCode.Text.Trim() + "%");
            }
            ...
            if (this.selSecVar.Value != null)
            {
                paraDict.Add("C_SEC_VAR_CODE", "%" + this.selSecVar.Value + "%");
            }
        ...
        private void Frm_SEC_BASE_YQ_L_Load(object sender, EventArgs e)
        {
