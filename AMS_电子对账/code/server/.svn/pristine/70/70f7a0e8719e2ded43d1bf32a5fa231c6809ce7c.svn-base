create or replace procedure PRC_DEP_CHK_TRADE_IVT_HG
  ...
  v_checkCnt := 0;
   LOOP
      ...
      IF (v_field = 'C_CA_CODE') THEN
        v_quyCon := v_quyCon || ' (C_CA_CODE_SETT_DUE=''' || v_value || ''' OR  C_CA_CODE_SETT_FIRST= ''' || v_value || ''' ) AND';
      ELSE
   ...
   IF (LENGTH(v_quyCon) > 0) THEN
      ...
      v_td_type_con := ' AND C_TD_TYPE LIKE ''HGJY%'' ';
