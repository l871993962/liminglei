create or replace procedure PRC_DEP_CHK_CA_CODE_FWTRADE
  ...
  v_checkCnt := 0;
   ...
   LOOP
      ...
      IF (v_field = 'C_CA_CODE') THEN
        v_quyCon := v_quyCon || '( C_CA_CODE_OT=''' || v_value || ''' OR C_CA_CODE_BAIL=''' || v_value || ''' OR C_CA_CODE_SETT_DUE=''' || v_value || ''' OR C_CA_CODE_SETT_FIRST=''' || v_value || ''') AND';
      ELSE
...
end PRC_DEP_CHK_CA_CODE_FWTRADE;
