create or replace procedure PRC_DPD_CHK_STOCKINFO
...
begin
  ...
  v_checkCnt := 0;
   LOOP
      ...
      IF (v_field = 'C_SEC_CODE') THEN
        v_quyCon := v_quyCon || 'C_SEC_CODE_TRG=''' || v_value || ''' AND';
      ELSE
   ...
   IF (LENGTH(v_quyCon) > 0) THEN
      v_sec_type_con := ' AND C_SEC_VAR_CODE LIKE ''GP%'' ';
      v_quyCon := SUBSTR (v_quyCon, 1, (LENGTH(v_quyCon) -4));
      v_quyConComp := SUBSTR (v_quyConComp, 1, (LENGTH(v_quyConComp) -4));

      v_sql := 'SELECT COUNT (' || v_field || ') FROM ' || v_dep_table_name || v_quyCon || v_sec_type_con;
      ...
      v_sql := 'SELECT COUNT (' || v_field || ') FROM ' || v_chp_table_name ||  v_quyConComp  || v_sec_type_con;
      dbms_output.put_line(v_sql);
