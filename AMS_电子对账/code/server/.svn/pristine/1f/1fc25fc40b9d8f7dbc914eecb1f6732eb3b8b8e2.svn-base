create or replace procedure PRC_DEP_CHK_CA_CODE_FURTRADE
  ...
  v_checkCnt := 0;
   ...
   LOOP
      ...
      IF (v_field = 'C_CA_CODE') THEN
        v_quyCon := v_quyCon || ' C_CA_CODE_BAIL = ''' || v_value || ''' AND';
      ELSE
   ...
   IF (LENGTH(v_quyCon) > 0) THEN
      v_quyCon := SUBSTR (v_quyCon, 1, (LENGTH(v_quyCon) -4)) + ' AND C_TD_CODE = ''QHJY'' ';
      v_quyConComp := SUBSTR (v_quyConComp, 1, (LENGTH(v_quyConComp) -4)) + ' AND C_TD_CODE = ''QHJY'' ';

      v_sql := 'SELECT COUNT (' || v_field || ') FROM ' || v_dep_table_name || v_quyCon;
      ...
      v_sql := 'SELECT COUNT (' || v_field || ') FROM ' || v_chp_table_name ||  v_quyConComp;
      dbms_output.put_line(v_sql);
...
end PRC_DEP_CHK_CA_CODE_FURTRADE;
/
