CREATE OR REPLACE Function getDay_MKT(C_MKT_Code     varchar2,
                                      D_Stand_Date   date,
                                      N_Interval_Day int,
  ...
  */
  ...
  D_Tmp      Date;
  N_Count    int := 1;
  C_MKT      varchar2(20) := C_MKT_Code;
  N_Interval int := NVL(N_Interval_Day, 0);
begin
  D_Return := D_Stand_Date;
  if (C_MKT = null or N_Interval = 0) then
    return D_Return;
  end if;
  /*判断参数是否合法*/
        ...
        begin
          if (N_Interval_Day >= 1) then
            select count(*)
             ...
             where a.C_HDay_Code in
                   (select C_HDay_Code
                      from T_P_BI_MKT b
                     where b.C_MKT_Code = C_MKT)
