CREATE OR REPLACE PROCEDURE PRC_CHK_DC_CODE_DATA
IS
   v_sql         VARCHAR2 (4000);
   ...
   IS
        SELECT      'SELECT '''
                 ...
                 || '"),0) as cnt FROM "'
                 || t1.table_name
                 || '" t WHERE  NOT EXISTS(SELECT * FROM T_S_DC_CURY m WHERE t.C_DC_CODE = m.C_DC_CODE) and C_DC_CODE <> '' '' AND C_DC_CODE <> ''***'''
                 --|| 'AND t."'
                 --|| t1.COLUMN_NAME
                 --|| '" <> '' '''
                    AS str,
         ...
         WHERE   NOT EXISTS
                 ...
                 AND t1.COLUMN_NAME = 'C_DC_CODE'
      ORDER BY   t1.Table_Name, t1.Column_ID;
BEGIN

   c_Exe_Type := 'CHK_DC_CODE';
   c_Exe_Prc  := 'PRC_CHK_DC_CODE_DATA';

   DELETE FROM T_CHK_EXCPTION_DATA WHERE C_EXE_TYPE =  'CHK_DC_CODE';
