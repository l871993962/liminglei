CREATE OR REPLACE PROCEDURE PRC_CHK_TABIDX_OFIVE_DATA
IS
   v_sql         VARCHAR2 (4000);
   ...
   IS
    ...
    SELECT table_name as c_table_name,count(*) as cnt FROM USER_INDEXES GROUP BY table_name HAVING COUNT(*) >= 5;


BEGIN

   c_Exe_Type := 'CHK_TABIDX_OFIVE';
   c_Exe_Prc  := 'PRC_CHK_TABIDX_OFIVE_DATA';

   DELETE FROM T_CHK_EXCPTION_DATA WHERE C_EXE_TYPE =  'CHK_TABIDX_OFIVE';
