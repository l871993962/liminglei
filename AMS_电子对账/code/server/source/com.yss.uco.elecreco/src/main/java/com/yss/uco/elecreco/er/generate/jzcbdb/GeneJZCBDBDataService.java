package com.yss.uco.elecreco.er.generate.jzcbdb;
...
public class GeneJZCBDBDataService extends GeneElecDataService{
	/**
	 * 报表管理（自定义列）
	...
	protected static final String C_FILE_TYPE = "1903";
	@Override
	public Map<String, Object> geneElecData() throws Exception {
		...
		try {
			...
			this.getAssCode();
			this.deleteHistory();
			count += this.geneJZCBDData();
		} catch (Throwable e) {
			result = "Fail";
			resultDetail = e.getMessage();
			logger.error("生成净资产变动表数据出错：" + e.getMessage(), e);
		}
		
		if(count > 0){
			this.geneErInfo(C_FILE_TYPE, c_rpt_type);
			result = this.fsn;
			resultDetail = "电子对账净资产变动表数据生成成功！";
	...
	private void deleteHistory() throws Exception {
		...
		try {
			...
			int index = 1;
			buffer.append("DELETE FROM T_D_ER_JZCBD A WHERE A.C_ASS_CODE = ? AND A.C_RPT_TYPE = ? AND A.D_START_DATE = ? AND A.D_END_DATE = ? ");
	...
	/**
	 * 插入净资产变动表T_D_ER_JZCBD预处理sql语句
	 * @return
	 */
	protected String getInsertSql(){
		StringBuilder builder = new StringBuilder();
		builder.append("INSERT INTO T_D_ER_JZCBD ");
...
//		builder.append(" VALUES(SEQU_D_ER_JZCBD.NEXTVAL,'").append(this.fsn).append("','").append(C_FILE_TYPE).append("','").append(c_rpt_type);
//		builder.append("','").append(this.assCode);
//		builder.append("','").append(this.startDate);
//		builder.append("','").append(this.endDate).append("',");
		builder.append(" VALUES(SEQU_D_ER_JZCBD.NEXTVAL,?,?,?,?,?,?,?,");
		builder.append("?,?,?)");
	 ...
	 * @throws Exception
	 */
	protected int geneJZCBDData() throws Exception {
		PreparedStatement pst = null;
		...
		try {
			...
			if(reportManager.isCustomCol())
			{
				...
				dataSql = getJZCBDCustomColSql(reportManager.getReortColIndexByElecColIndex());
			}else
			{
				dataSql = getJZCBDSql();
			}
	 ...
	 * @return
	 */
	protected String getJZCBDSql(){
		ArrayList<Integer> cols = new ArrayList<Integer>();
		cols.add(1);
		cols.add(2);
		return this.getJZCBDCustomColSql(cols);
	}
	 ...
	 * @return
	 */
	protected String getJZCBDCustomColSql(List<Integer> cols){
		StringBuilder builder = new StringBuilder();

		builder.append("select a.c_zb_code,a.c_zb_name, a.N_CUR_VALUE, b.N_TOL_VALUE, a.n_row_num from\n");
		builder.append("(select a.c_zb_code,a.c_zb_name,nvl(b.c_data,'0') as N_CUR_VALUE,b.n_row_num, b.n_col_num\n"); 
		...
		builder.append("join\n"); 
		builder.append("(select a.c_zb_code,nvl(b.c_data,'0') as N_TOL_VALUE,b.n_row_num, b.n_col_num\n"); 
		//STORY53570嘉实基金-电子对账-月报 产品分类配置
		//builder.append("  from t_z_bi_rela a right join(\n"); 
	...
	/**
	 * 
	 *C_INDEX_CODE, N_BEGIN_VALUE, N_END_VALUE
	 * @param pst
	 * @param rs
	 * @throws SQLException
	 */
...
//		ElecPerRela perRelaPojo = this.getPerRelaByCode(rs.getString("C_ZB_CODE"));
		...
		switch (dataType) {
		default:
			pst.setString(index++, rs.getString(1));//C_ZB_CODE
			pst.setString(index++, rs.getString(2));//C_ZB_NAME
