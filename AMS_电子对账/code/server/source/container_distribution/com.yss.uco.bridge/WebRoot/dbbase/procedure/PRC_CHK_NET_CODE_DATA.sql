CREATE OR REPLACE PROCEDURE PRC_CHK_NET_CODE_DATA
IS
   v_sql         VARCHAR2 (4000);
   ...
   IS
        SELECT      'SELECT '''
                 ...
                 || '"),0) as cnt FROM "'
                 || t1.table_name
                 || '" t WHERE  NOT EXISTS(SELECT * FROM T_P_BI_SALES_NET m WHERE t.C_NET_CODE = m.C_NET_CODE) '
                 || 'AND t."'
                 || t1.COLUMN_NAME
                 || '" <> '' '''
                    AS str,
         ...
         WHERE   NOT EXISTS
                 ...
                 AND t1.COLUMN_NAME = 'C_NET_CODE'
      ORDER BY   t1.Table_Name, t1.Column_ID;
BEGIN

   c_Exe_Type := 'CHK_NET_CODE';
   c_Exe_Prc  := 'PRC_CHK_NET_CODE_DATA';

   DELETE FROM T_CHK_EXCPTION_DATA WHERE C_EXE_TYPE =  'CHK_NET_CODE';
