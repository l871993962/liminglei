create or replace procedure PRC_DEP_CHK_CA_CODE_FUTBAIL
  ...
  v_checkCnt := 0;
   ...
   LOOP
      ...
      IF (v_field = 'C_CA_CODE') THEN
        v_quyCon := v_quyCon || ' ( C_CA_CODE=''' || v_value || ''' OR C_CA_CODE_BAIL = ''' || v_value || ''' ) AND';
      ELSE
...
end PRC_DEP_CHK_CA_CODE_FUTBAIL;
