create or replace procedure PRC_DEPENDANCE_CHECK_PRODUCT(tableName in varchar2,
                                                         fields    in varchar2,
  ...
  /*
  *产品计划管理字段 C_PL_CODE
  * 和资产池表字段 C_POOL_CODE的转化
  * 当删除产品计划管理数据验证是否与资产池表有引用关系
  */
...
begin
  ...
  LOOP
    ...
    IF (instr(v_quyconbase, 'WHERE') = 0) THEN
      v_quyconbase := ' WHERE ';
    END IF;

    IF (v_field = 'C_PL_CODE') THEN
      v_quyCon := v_quyCon || 'C_POOL_CODE=''' || v_value || ''' AND ';
    ELSE
      v_quyCon := v_quyCon || v_field || '=''' || v_value || ''' AND ';
    END IF;

    v_quyconbase := v_quyconbase || v_field || '=''' || v_value ||
                    ''' AND ';
  ...
  IF (LENGTH(v_quyCon) > 0) THEN
    ...
    v_quyconbase := SUBSTR(v_quyconbase, 1, (LENGTH(v_quyconbase) - 4));
    v_sql        := 'SELECT COUNT (*) FROM ' || v_dep_table_name ||
                    v_quyCon;
    ...
    v_sql := 'SELECT COUNT (*) FROM ' || v_chp_table_name || v_quyconbase;
    dbms_output.put_line(v_sql);
...
end PRC_DEPENDANCE_CHECK_PRODUCT;
