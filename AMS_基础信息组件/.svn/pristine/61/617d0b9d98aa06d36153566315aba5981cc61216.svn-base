CREATE OR REPLACE PROCEDURE PRC_CHK_CURY_PAIR_DATA
IS
   v_sql         VARCHAR2 (4000);
   ...
   IS
        SELECT      'SELECT '''
                 ...
                 || '"),0) as cnt FROM "'
                 || t1.table_name
                 || '" t WHERE  NOT EXISTS(SELECT * FROM t_p_bi_cury_pair m WHERE t.C_CURY_PAIR_CODE = m.C_CURY_PAIR_CODE) '
                 || 'AND t."'
                 || t1.COLUMN_NAME
                 || '" <> '' '''
                    AS str,
         ...
         WHERE   NOT EXISTS
                 ...
                 AND t1.COLUMN_NAME = 'C_CURY_PAIR_CODE'
      ORDER BY   t1.Table_Name, t1.Column_ID;
BEGIN

   c_Exe_Type := 'CHK_CURY_PAIR';
   c_Exe_Prc  := 'PRC_CHK_CURY_PAIR_DATA';

   DELETE FROM T_CHK_EXCPTION_DATA WHERE C_EXE_TYPE =  'CHK_CURY_PAIR';
