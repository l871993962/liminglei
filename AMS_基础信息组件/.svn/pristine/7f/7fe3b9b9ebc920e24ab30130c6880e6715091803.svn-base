create or replace procedure PRC_DEP_CHK_NET_CODE_TASETT
...
begin
  ...
  v_checkCnt := 0;
   LOOP
      ...
      IF (v_field = 'C_NET_CODE') THEN
        v_quyConComp := v_quyConComp || ' C_SELL_NET_CODE =''' || v_value || ''' AND';
      ELSE
   ...
   IF (LENGTH(v_quyCon) > 0) THEN
      ...
      v_sql := 'SELECT COUNT (*) FROM ' || v_dep_table_name || v_quyConComp;
      ...
      v_sql := 'SELECT COUNT (*) FROM ' || v_chp_table_name || v_quyCon;
      dbms_output.put_line(v_sql);
...
end PRC_DEP_CHK_NET_CODE_TASETT;
