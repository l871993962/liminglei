create or replace procedure PRC_DEP_CHK_CURRUNCY_EXH
 ...
 v_checkCnt := 0;
   LOOP
      ...
      IF(v_field = 'C_CA_CODE') THEN
        v_quyCon := v_quyCon || ' C_CA_CODE_SETT_DUE=''' || v_value || ''' OR C_CA_CODE_SETT_DUE_I=''' || v_value || ''' AND';
      ELSE
   ...
   IF (LENGTH(v_quyCon) > 0) THEN
      ...
      v_sql := 'SELECT COUNT (*) FROM ' || v_chp_table_name ||  ' WHERE C_CA_CODE = ''' || v_value || ''' AND N_CHECK_STATE = 1 ';
      dbms_output.put_line(v_sql);
