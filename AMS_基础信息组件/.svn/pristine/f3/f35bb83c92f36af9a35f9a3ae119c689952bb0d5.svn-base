CREATE OR REPLACE PROCEDURE PRC_CHK_CHECK_STATE_DATA
IS
   v_sql         VARCHAR2 (4000);
   ...
   IS
        SELECT      'SELECT '''
                 ...
                 || '"),0) as cnt FROM "'
                 || t1.table_name
                 || '" t WHERE N_CHECK_STATE NOT IN (0,1)'
                    AS str,
         ...
         WHERE   NOT EXISTS
                 ...
                 AND t1.COLUMN_NAME = 'N_CHECK_STATE'
      ORDER BY   t1.Table_Name, t1.Column_ID;
BEGIN

   c_Exe_Type := 'CHK_CHECK_STATE';
   c_Exe_Prc  := 'PRC_CHK_CHECK_STATE_DATA';

   DELETE FROM T_CHK_EXCPTION_DATA WHERE C_EXE_TYPE =  'CHK_CHECK_STATE';
