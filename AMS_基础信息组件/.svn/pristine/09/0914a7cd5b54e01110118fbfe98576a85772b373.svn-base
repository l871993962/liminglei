 ...
 *
 */
public class MrInfoSqlBuilder implements SQLBuilder{
	...
	public String getTableName(DBNameResolver dbNameResolver) {
		return dbNameResolver.getTableName(MrInfoTableName.table);
	}

	@Override
	public String getColumnNameByProperty(DBNameResolver dbNameResolver,
			String name) {
		return dbNameResolver.getColumnName(MrInfoColumnName.valueOf(name));
	}
	...
	public String getLogSequenceName(DBNameResolver dbNameResolver) {
		return dbNameResolver.getLogSequenceName(MrInfoTableName.table);
	}
	...
	public String getQueryConditionSql(List<String> paraNameList)
		...
		buf.append(" SELECT * FROM T_D_ER_MRINFO A ");
		if (valueFieldbuf.length() > 0) {
			buf.append(" WHERE ").append(valueFieldbuf);
		}
		buf.append(" ORDER BY C_DV_SWITCH_MARK, N_ORDER ");
		return buf.toString();
	}
