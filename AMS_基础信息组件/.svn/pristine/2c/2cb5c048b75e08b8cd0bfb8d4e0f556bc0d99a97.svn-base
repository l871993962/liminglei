CREATE OR REPLACE PROCEDURE PRC_CHK_MKT_CODE_DATA
------------------------------------------------------------------------
-- author         date                 status     desc
-- fazmm          20121031             Add        检查交易市场是否存在
------------------------------------------------------------------------
IS
   v_sql         VARCHAR2 (4000);
   ...
   IS
        SELECT      'SELECT '''
                 ...
                 || '"),0) as cnt FROM "'
                 || t1.table_name
                 || '" t WHERE  NOT EXISTS(SELECT * FROM T_P_BI_MKT m WHERE t.c_mkt_code = m.c_mkt_code) '
                 || 'AND t."'
                 || t1.COLUMN_NAME
                 || '" <> '' '''
                    AS str,
         ...
         WHERE   NOT EXISTS
                 ...
                 AND t1.COLUMN_NAME = 'C_MKT_CODE'
      ORDER BY   t1.Table_Name, t1.Column_ID;
BEGIN

   c_Exe_Type := 'CHK_MKT_CODE';
   c_Exe_Prc  := 'PRC_CHK_MKT_CODE_DATA';

   DELETE FROM T_CHK_EXCPTION_DATA WHERE C_EXE_TYPE =  'CHK_MKT_CODE';
