CREATE OR REPLACE PROCEDURE PRC_CHK_CA_CODE_DATA
IS
   v_sql         VARCHAR2 (4000);
   ...
   IS
        SELECT      'SELECT '''
                 ...
                 || '"),0) as cnt FROM "'
                 || t1.table_name
                 || '" t WHERE  NOT EXISTS(SELECT * FROM T_P_AB_CASH_ACC m WHERE t.C_CA_CODE = m.C_CA_CODE) AND t.C_CA_CODE <> '' '''
                    AS str,
         ...
         WHERE   NOT EXISTS
                 ...
                 AND t1.COLUMN_NAME = 'C_CA_CODE'
      ORDER BY   t1.Table_Name, t1.Column_ID;
BEGIN

   c_Exe_Type := 'CHK_CA_CODE';
   c_Exe_Prc  := 'PRC_CHK_CA_CODE_DATA';

   DELETE FROM T_CHK_EXCPTION_DATA WHERE C_EXE_TYPE =  'CHK_CA_CODE';
