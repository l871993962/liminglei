CREATE OR REPLACE Function getDay_SEC(C_SEC_Code     varchar2,
                                      D_Stand_Date   date,
                                      N_Interval_Day int,
                                      C_Type         varchar2) return Date as
  /*函数说明：通过市场代码获取指定的日期[查找基准日期前/后N自然/工作日的日期]
  ...
  */
  ...
  D_Tmp      Date;
  N_Count    int := 1;
  C_SEC      varchar2(20) := C_SEC_Code;
  N_Interval int := NVL(N_Interval_Day, 0);
begin
  D_Return := D_Stand_Date;
  if (C_SEC = null or N_Interval = 0) then
    return D_Return;
  end if;
  /*判断参数是否合法*/
        ...
        begin
          if (N_Interval_Day >= 1) then
            select count(*)
              ...
              from T_P_BI_HDAY_SUB c, T_P_SV_SEC_BASE a, T_P_BI_MKT b
             where a.c_Mkt_Code = b.c_Mkt_Code
               and c.C_HDAY_CODE = b.C_HDAY_CODE
               and a.C_SEC_CODE = C_SEC
               and c.D_HDay > D_Start
               and c.D_HDay <= D_End
               and c.c_Date_Type = 'H'
