...
public class FileUtil {
	...
	public void close() {
		try {
			...
			logger.log("关闭出错:" + ex.getMessage(), ex);
		}
	}
	...
	public static void writeFile(String path, String content) {
		...
		try {
			...
			if (!f.createNewFile()) {
				logger.log("文件创建失败！文件名:"+path);
			}
			output = new BufferedWriter(new FileWriter(f));
			output.write(content);
			output.close();
		} catch (Exception e) {
			logger.log("写入数据出错:" + e.getMessage(), e);
		}finally{
			if(output != null){
				try {
					output.close();
				} catch (IOException e) {
					logger.log("关闭出错:" + e.getMessage(), e);
				}
			}
		}
	}
...
//					line++;
			...
			logger.log("读取文件出错:" + e.getMessage(), e);
		} finally {
			if (reader != null) {
				try {
					reader.close();
