...
/**
 * 核算业务项字典T_S_DVA_ITEM dao
 *
 */
public class DavItemDao extends GeneralDao{

	public DavItemDao(DbPool pool, SQLBuilder sqlBuilder) {
		super(pool, sqlBuilder);
	}

	/**
	 * 根据条件参数获取核算业务项字典T_S_DVA_ITEM的数据信息 并进行分页处理
...
//			}
	 ...
	 * @return
	 */
	public List<DvaItem> getAllDataList() throws Exception {
		List<DvaItem> pojoList = new ArrayList<DvaItem>();
		...
		DavItemSqlBuilder dsServiceBuilder = null;
		DvaItem t = null;
		try {
			dsServiceBuilder = new DavItemSqlBuilder();
			...
			while (rs.next()) {
				t = rsTools.ResultToBeanGeneric(rs, DvaItem.class);
				pojoList.add(t);
			}
	 ...
	 * @return
	 */
	public DvaItem getDataByCode(String code) throws Exception {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	 ...
	 * @return
	 */
	public List<DvaItem> getDataListByTypes(String[] types) throws Exception {
		List<DvaItem> pojoList = new ArrayList<DvaItem>();
	...
	/**
	 * 获取核算业务项字典T_S_DVA_ITEM的所有数据（值包含核算项目代码，核算项目名称）
	 * @return
	 */
	public HashMap<String, String> getKeyConvertMap() throws Exception {
		...
		try {
			...
			while (rs.next()) {
				t = rsTools.ResultToBeanGeneric(rs, DvaItem.class);
				keyValueMap.put(t.getC_DVA_ITEM_CODE(), t.getC_DVA_ITEM_NAME());
			}
	 ...
	 * @return
	 */
	public List<DvaItem> getDataListByKeys(String[] keys) throws Exception {
		List<DvaItem> pojoList = new ArrayList<DvaItem>();
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public List<DvaItem> getTreeViewForDayfItems() throws Exception {
		...
		try {
			...
			sql = dsServiceBuilder.getDvaItemTreeViewSql();
...
//			rsTools = new ResultSetTools(dbNameResolver, sqlbuilder);
			...
			conn = this.loadNewConnection();
			//conn.setAutoCommit(false);
			sql = dsServiceBuilder.getTreeViewQuerySql(paraNameList);

			pstmt = conn.prepareStatement(sql);
			logger.debug(sql);
			clazz = PojoLoader.getPojoClassById("Zzywgl_A",bundleContext);
			rs = pstmt.executeQuery();
